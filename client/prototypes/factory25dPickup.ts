...
import {factoryWorldPoint,factoryScenePoint,recoverFactoryPosition,toFactoryWorld,fromFactoryWorld,type FactoryRoom} from '@shared/factory25d-layout';
...
export function createPickupMotion(mesh:THREE.Mesh,avatar:AvatarConfig){
  ...
  let phase:'idle'|'held'|'falling'='idle',last=performance.now(),lifted=false,liftedAt=0;
  ...
  return {get active(){return phase!=='idle';},get shadowAirborne(){return lifted&&phase!=='idle';},get stage(){return phase==='held'?(lifted?'lifted':'pulling'):phase;},
    update(camera:THREE.Camera,canvas:HTMLCanvasElement,pointer?:{x:number;y:number}){
      ...
      if(pointer){
        ...
        if(ray.ray.intersectPlane(plane,target)){
          ...
          if(!lifted&&hanging.y<planted.y){lifted=true;liftedAt=now;}
        ...
        if(phase==='held'){
          ...
          mesh.getWorldPosition(world);plane.constant=-world.z;
        ...
        if(phase==='falling'){
          ...
          if(body.y>=-home.y/UNIT&&body.vy>=0){phase='idle';mesh.position.copy(home);}
          else mesh.position.set(body.x*UNIT,-body.y*UNIT,mesh.position.z);
        }
      }
      ...
      return phase;
    },dispose(){material.transparent=transparent;material.depthTest=depthTest;material.depthWrite=depthWrite;mesh.renderOrder=renderOrder;fold.dispose();}};
}
...
export function createStaffPickup(mesh:THREE.Mesh,button:HTMLButtonElement,canvas:HTMLCanvasElement,avatar:AvatarConfig){
  ...
  button.addEventListener('pointerdown',e=>{if(e.button!==0||!mesh.visible)return;press={id:e.pointerId,x:e.clientX,y:e.clientY};if(texture)pressPose.copy(texture.offset);suppress=false;button.setPointerCapture(e.pointerId);e.stopPropagation();},options);
  ...
  return {get shadowAirborne(){return motion.shadowAirborne;},get airborne(){return motion.stage==='lifted';},get held(){return !!pointer;},get busy(){return !!pointer||motion.active||returning;},
    update(camera:THREE.Camera){
      ...
      if(returning){dropped.copy(mesh.position);if(phase==='idle'){
        ...
        if(distance<.035){returning=false;mesh.position.copy(home);}else{dropped.addScaledVector(delta,Math.min(1,2.1*dt/distance));mesh.position.copy(dropped);}
      }}
    },dispose(){end();abort.abort();motion.dispose();}};
}
