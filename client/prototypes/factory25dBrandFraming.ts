...
export function brandFraming(width: number, height: number) {
...
export function brandClosePose(focus: THREE.Vector3, rotation: THREE.Quaternion,
  ...
  const frame = brandFraming(width, height), aspect = width / Math.max(1, height);
  const view = new THREE.PerspectiveCamera();
  view.position.copy(focus).add(new THREE.Vector3(.35, .8, 3).applyQuaternion(rotation));
