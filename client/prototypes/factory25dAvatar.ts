...
import { resolveAvatar, drawCharacter, hexToInt, type AvatarEyes } from '../rendering/avatarPainter';
...
export function avatarSheet(avatar: AvatarConfig, animations: readonly string[] = AVATAR_ANIMATIONS, eyes: readonly (AvatarEyes | undefined)[] = [undefined]) {
  const colors = resolveAvatar(avatar), key = JSON.stringify([colors, animations, eyes]);
  ...
  for (const [expression, eye] of eyes.entries()) for (const [row, animation] of animations.entries()) for (let frame = 0; frame < 4; frame++)
    drawCharacter(ctx, (expression * 4 + frame) * 32, row * 32, 32, hexToInt(colors.shirtColor), animation, frame, colors, eye);
  ...
  const feet = animations.map((_, row) => Array.from({length:4}, (_, frame) => {
    for (let y = 31; y >= 0; y--) for (let x = 0; x < 32; x++)
      if (pixels[((row * 32 + y) * canvas.width + frame * 32 + x) * 4 + 3] >= 20) return y + 1;
    return 32;
  }));
