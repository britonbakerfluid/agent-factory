...
import { factoryRoomAt, factoryScenePoint, GARAGE_LEVEL } from '@shared/factory25d-layout';
...
export function createFactoryTombstones(factory: THREE.Scene, patio: THREE.Scene, canvas: HTMLCanvasElement) {
  ...
  return {
    configureGarage(config: { scene: THREE.Scene; isVisible: () => boolean }) { garage = config; },
    update(states: Map<string, TombstoneState>, environment: EnvironmentType, now: number, camera: THREE.Camera,
      ...
      for (const state of states.values()) {
        ...
        const point = projectPosition(state.position, environment), room = factoryRoomAt(point), local = factoryScenePoint(point), outside = room === 'patio', parent = room === 'garage' ? garage?.scene ?? factory : outside ? patio : factory;
        if (grave.group.parent !== parent) parent.add(grave.group);
        const floorY = room === 'garage' ? GARAGE_LEVEL + .018 : outside ? patioFloorHeight(point) + .018 : floor({ x: point.x, z: point.z - 1.95 });
        grave.group.position.set(local.x, floorY, local.z);
        ...
        grave.label.update(grave.group, floorY, camera, canvas, room === 'garage' ? garage?.isVisible() ?? false : outside ? showPatio : showFactory, room === 'factory' ? occluder : undefined);
      }
