...
/** One lease per avatar: top-level agents key on sessionId, subagents on sessionId:agentId. */
export function grabKey(target: GrabTarget): string {
  return target.agentId ? `${target.sessionId}:${target.agentId}` : target.sessionId;
}
...
export function parseGrabTarget(msg: { sessionId?: unknown; agentId?: unknown }): GrabTarget {
  const target: GrabTarget = { sessionId: String(msg.sessionId || '') };
  if (msg.agentId) target.agentId = String(msg.agentId);
  return target;
}
 ...
 * the same dangling avatar. Nothing here touches AgentSession, so a grab never persists.
 */
export class GrabManager {
  ...
  begin(socket: WebSocket, username: string | undefined, target: GrabTarget, x: number, y: number): boolean {
    ...
    const lease: GrabLease = {
      ...
      x: clamp(x, CONTROL_WORLD_BOUNDS.minX, CONTROL_WORLD_BOUNDS.maxX),
      y: clamp(y, CONTROL_WORLD_BOUNDS.minY, CONTROL_WORLD_BOUNDS.maxY),
  ...
  move(socket: WebSocket, username: string | undefined, target: GrabTarget, x: number, y: number): boolean {
    ...
    lease.x = clamp(x, CONTROL_WORLD_BOUNDS.minX, CONTROL_WORLD_BOUNDS.maxX);
    lease.y = clamp(y, CONTROL_WORLD_BOUNDS.minY, CONTROL_WORLD_BOUNDS.maxY);
    lease.dirty = true;
    if (timestamp - lease.lastBroadcastAt >= MAX_BROADCAST_RATE_MS) this.flush(lease, timestamp);
    return true;
  }

  end(socket: WebSocket, username: string | undefined, target: GrabTarget, x: number, y: number): boolean {
    const lease = this.authorizedLease(socket, username, target);
  ...
  private invalidReason(target: GrabTarget, session: AgentSession): string | null {
    ...
    if (target.agentId) {
      if (!session.subagents.some(sub => sub.agentId === target.agentId)) return 'Subagent finished';
      return null;
    }
  ...
  private toTarget(lease: GrabLease): GrabTarget {
    return lease.agentId ? { sessionId: lease.sessionId, agentId: lease.agentId } : { sessionId: lease.sessionId };
  }
