...
beforeEach(() => {
  ...
  vi.stubGlobal('document', {
    ...
    body: { classList: { add: (c: string) => classes.add(c), remove: (c: string) => classes.delete(c), has: (c: string) => classes.has(c) } },
    addEventListener: (event: string, fn: Listener) => {
...
describe('client GrabManager gestures', () => {
  ...
  it('requests a lease on drag and only lifts once the server confirms', () => {
    ...
    expect(document.body.classList.has('is-grabbing')).toBe(true);
  });
  ...
  it('drops the avatar where the pointer is released and tells the server', () => {
    ...
    expect(h.manager.holding).toBeNull();
    expect(document.body.classList.has('is-grabbing')).toBe(false);
  });
...
describe('client GrabManager cancellation and remote state', () => {
  ...
  it('mirrors other viewers\' grabs onto sprites, including subagents, and ignores its own echo', () => {
    const h = harness();
    const sub = { sessionId: 'alice-1', agentId: 'sub-1' };
    h.manager.handleMessage({ type: 'grab_update', grab: { ...sub, username: 'carol', x: 10, y: 20 } });
    expect(h.agents.applyRemoteGrab).toHaveBeenCalledWith(sub, { x: 10, y: 20 });
    h.manager.handleMessage({ type: 'grab_release', ...sub, x: 12, y: 22, reason: 'Released' });
    expect(h.agents.releaseGrab).toHaveBeenCalledWith(sub, { x: 12, y: 22 });
  ...
  it('compares targets by session and subagent id', () => {
    expect(sameGrabTarget({ sessionId: 'a' }, { sessionId: 'a' })).toBe(true);
    expect(sameGrabTarget({ sessionId: 'a' }, { sessionId: 'a', agentId: 's' })).toBe(false);
    expect(sameGrabTarget({ sessionId: 'a', agentId: 's' }, { sessionId: 'a', agentId: 's' })).toBe(true);
  });
});
