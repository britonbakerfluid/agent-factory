...
const calls = vi.hoisted(() => ({ render: vi.fn(), bear: vi.fn(), elk: vi.fn(), birds: vi.fn(), climbers: vi.fn(), canoe: vi.fn() }));
...
it('does no landscape rendering or visitor animation offscreen, then applies the latest weather on return', () => {
  ...
  expect(calls.canoe).toHaveBeenCalledOnce();
  expect(calls.elk).toHaveBeenCalledOnce();
  ...
  expect(calls.canoe).toHaveBeenLastCalledWith(expect.closeTo(.01), snow, true, false);
  expect(calls.elk).toHaveBeenLastCalledWith(expect.closeTo(.01), snow, true, false);
  ...
  expect(calls.canoe).toHaveBeenLastCalledWith(expect.closeTo(.016), snow, true, false);
});
