...
import { GrabManager, grabKey, parseGrabTarget } from '../server/grab-manager.js';
...
function setup() {
  ...
  state.onStateChange((type, data) => {
    if (type === 'update' && data.agent) grabs.syncSession(data.agent);
    if (type === 'remove' && data.sessionId) grabs.releaseSession(data.sessionId, 'Agent session ended');
  });
  const viewer = socket();
...
describe('GrabManager authorization', () => {
  ...
  it('rejects invalid coordinates and clamps out-of-room pointers', () => {
    ...
    expect(grabs.activeGrabs()[0]).toMatchObject({ x: CONTROL_WORLD_BOUNDS.minX, y: CONTROL_WORLD_BOUNDS.maxY });
  });
});
...
describe('GrabManager leases and conflicts', () => {
  ...
  it('never writes grab state into the persisted session', () => {
    const { state, grabs, join } = setup();
    hook(state, 'SessionStart', 'alice-1', 'alice');
...
describe('GrabManager subagents', () => {
  it('grabs subagents by agentId and drops them when the subagent stops', () => {
    const { state, grabs, join, viewer } = setup();
    hook(state, 'SessionStart', 'alice-1', 'alice');
    hook(state, 'SubagentStart', 'alice-1', 'alice', { agent_id: 'sub-1', agent_type: 'research' });
    const bob = join(socket());
    const target = { sessionId: 'alice-1', agentId: 'sub-1' };

    expect(grabs.begin(bob, 'bob', { sessionId: 'alice-1', agentId: 'ghost' }, 100, 100)).toBe(false);
    expect(grabs.begin(bob, 'bob', target, 100, 100)).toBe(true);
    expect(grabKey(target)).toBe('alice-1:sub-1');
    expect(ofType(viewer.sent, 'grab_update')[0].grab).toMatchObject({ agentId: 'sub-1', username: 'bob' });

    hook(state, 'SubagentStop', 'alice-1', 'alice', { agent_id: 'sub-1' });
    expect(grabs.activeGrabs()).toEqual([]);
    expect(ofType(viewer.sent, 'grab_release')[0]).toMatchObject({ agentId: 'sub-1', reason: 'Subagent finished' });
    expect(ofType(bob.sent, 'grab_result')).toContainEqual(expect.objectContaining({ action: 'end', agentId: 'sub-1' }));
  });

  it('keeps a subagent lease independent from the parent avatar lease', () => {
    const { state, grabs, join } = setup();
    hook(state, 'SessionStart', 'alice-1', 'alice');
    hook(state, 'SubagentStart', 'alice-1', 'alice', { agent_id: 'sub-1', agent_type: 'research' });
    const bob = join(socket());
    ...
    expect(grabs.begin(bob, 'bob', { sessionId: 'alice-1' }, 100, 100)).toBe(true);
    expect(grabs.begin(carol, 'carol', { sessionId: 'alice-1', agentId: 'sub-1' }, 100, 100)).toBe(true);
    expect(grabs.activeGrabs()).toHaveLength(2);
  });
});
...
describe('GrabManager cleanup', () => {
  ...
  it('releases every lease on a session when it ends or is removed', () => {
    const { state, grabs, join, viewer } = setup();
    hook(state, 'SessionStart', 'alice-1', 'alice');
    hook(state, 'SubagentStart', 'alice-1', 'alice', { agent_id: 'sub-1', agent_type: 'research' });
    const bob = join(socket());
    const carol = join(socket());
    grabs.begin(bob, 'bob', { sessionId: 'alice-1' }, 100, 100);
    grabs.begin(carol, 'carol', { sessionId: 'alice-1', agentId: 'sub-1' }, 100, 100);

    hook(state, 'SessionEnd', 'alice-1', 'alice');
    expect(grabs.activeGrabs()).toEqual([]);
    const releases = ofType(viewer.sent, 'grab_release');
    expect(releases.map(r => r.agentId)).toEqual(expect.arrayContaining([undefined, 'sub-1']));
  ...
  it('does not fight manual control: refuses controlled avatars and lets go when control begins', () => {
    ...
    controls.claim(alice, 'alice', 'alice-1', 100, 100);
  ...
  it('parses untrusted socket payloads into targets', () => {
    expect(parseGrabTarget({ sessionId: 'a', agentId: 'b' })).toEqual({ sessionId: 'a', agentId: 'b' });
