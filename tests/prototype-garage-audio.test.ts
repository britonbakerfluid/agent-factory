...
it('exposes opt-in YouTube volume independently of engine lifecycle', async () => {
  ...
  const audio = createFactoryAudio();
  expect(audio.musicPreferences()).toEqual({ enabled: false, volume: 40 });
  audio.enableMusic();
  for (let i = 0; i < 20; i++) await Promise.resolve();
  expect(audio.musicPreferences()).toEqual({ enabled: true, volume: 40 });
  for (let frame = 0; frame < 20; frame++) audio.garageEngine(undefined);
  expect(audio.musicPreferences().enabled).toBe(true);
  const volume = controls.get('#scene-volume')!;
  volume.value = '65'; volume.dispatchEvent(new Event('input'));
  expect(audio.musicPreferences().volume).toBe(40);
