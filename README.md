...
minutes, without saying goodbye. A report is only held for a session the server already
knows, reported by the installation that owns it; anything else is refused. Sessions are grouped by the server their working directory resolves to, so
[repository-aware overrides](#repository-aware-overrides) keep holding: a session only ever
reports to its own server. Reports require the same installation credential the hooks use --
unlike `/api/hooks`, an unsigned one is refused -- so only the machine that owns a session can
hold it open.
...
3. It authenticates and sends an allowlisted subset of the event to the server (fire-and-forget, never blocks Claude)
4. The server normalizes the payload again, derives an opaque installation owner, updates the authoritative world, and checkpoints it to libSQL
...
**Client -> Server:** `request_state`, `logout`, `control_claim`, `control_input`, `control_release`, `shoot`, `grab_start`, `grab_move`, `grab_end`, `emote`, `chat`
