...
1. Create a nonsecret JSON configuration based on `config/github.fluid.example.json`. Set `organization`, `repository` (`owner/name`), `baseBranch`, and `identities`. Each identity has a `githubLogin` and explicit `factoryUsernames` aliases. Ambiguous aliases are rejected. One repository and up to 25 identities are supported per deployment, keeping each hourly batch below GitHub's 30 search requests/minute limit. Counts are not automatically assigned using task names or similar-looking display names.
...
6. Install the app on the configured organization, choose **Only select repositories**, and select the configured repository (`fluid-mono` for Fluid). The worker requests a token narrowed to that repository and read-only pull requests even if installation access is broader.
...
The existing libSQL/Turso database stores live totals in `github_contribution_totals`, keyed by a hash of deployment URL, organization, repository, branch, and app ID. Changing these settings starts an empty scope. Old unscoped `contribution_totals` data is retained for rollback but is never imported into this integration. If `AF_CONTRIBUTIONS_CACHE_PATH` is set, each scope instead uses `<path>.<scope-hash>.json`; the old unscoped file is likewise ignored. Aliases are operator configuration, not learned from GitHub; removed identities are filtered out and browsers clear old aliases when configuration changes.

Use one worker per deployment for this initial hourly polling implementation. Multiple processes deduplicate within each process only and would multiply API traffic. Multi-repository aggregation, automatic org roster discovery, OAuth identity linking, and webhook-driven refresh are follow-ups.
